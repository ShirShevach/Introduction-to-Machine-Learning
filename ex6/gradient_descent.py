...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        len_w = f.weights.shape[0]
        min_obj = f.weights
        val_min = f.compute_output(X=X, y=y)
        sum_vec = np.zeros(len_w)
        for t in range(self.max_iter_):
            direction = -f.compute_jacobian(X=X, y=y)
            eta = self.learning_rate_.lr_step(t=t)
            old_w = np.array(f.weights)
            old_val = f.compute_output(X=X, y=y)
            new_w = f.weights + eta * direction
            f.weights = new_w
            if f.compute_output(X=X, y=y) < val_min:
                val_min = f.compute_output(X=X, y=y)
                min_obj = f.weights
            sum_vec += f.weights
            delta = np.linalg.norm(x=f.weights - old_w, ord=2)
            self.callback_(solver=self, weights=old_w, val=old_val,
                           grad=-direction, t=t, eta=eta, delta=delta)
            if delta < self.tol_:
                break

        if self.out_type_ == 'average':
            return sum_vec / len_w
