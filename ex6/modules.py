...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        # return np.linalg.norm(self.weights, 2)**2
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        m = X.shape[0]
        X_times_w = X@self.weights
        a = (y@X_times_w) - np.sum(np.log(1+np.exp(X_times_w)))
        return a/(-m)
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        m = X.shape[0]
        w = self.weights
        exp_X_times_w = np.exp(X@w)
        a = y - np.exp(exp_X_times_w)
        b = 1+exp_X_times_w
        return ((a/b)@X)/(-m)
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        # kwargs_fidelity = deepcopy(kwargs)
        X = kwargs['X']
        # if self.include_intercept_:
        #     extend_X = np.ones((X.shape[0], X.shape[1] + 1))
        #     extend_X[:, 1:] = X
        #     X = extend_X
        return self.fidelity_module_.compute_output(X=X, y=kwargs['y']) + \
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        kwargs_fidelity = kwargs
        X = kwargs_fidelity["X"]
        # if self.include_intercept_:
        #     extend_X = np.ones((X.shape[0], X.shape[1] + 1))
        #     extend_X[:, 1:] = X
        #     kwargs_fidelity["X"] = extend_X
        return self.fidelity_module_.compute_jacobian(X=X, y=kwargs['y']) + \
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        if self.include_intercept_:
            w = weights[1:]
            self.regularization_module_.weights_ = w
        else:
